...
class File:
   ...
   def __init__(self, filename):
      ...
      if(self.store_meta_data.find('persist::gpstore/1.1') == 0) or (self.store_meta_data.find('persist::gpstore/3.0') == 0):
      ...
      elif(self.store_meta_data.find('amps-store-v1.0') == 0):
         ...
         while(self.file):
            ...
            # Pick-up the remaining records
               ...
               yield Record40({ 'crc':self.crc, 'flags': self.flags, 'data_size':self.data_size, 'allocated': self.allocated,
                     ...
                     'client_seq': self.client_seq, 'local_txid': self.local_txid , 'data':'','total_offset': self.slab_offset+self.file_offset})
            if (self.slab_size > 0):
