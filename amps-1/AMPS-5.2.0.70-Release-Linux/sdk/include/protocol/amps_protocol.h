...
struct amps_protocol_output_message
{
  ...
  uint32_t                               request_one;           // 28 - if true (non-zero) then only return a single segment
...
struct amps_output_message_sow_data
{
  ...
  amps_message_segment                  segment;                   // 144
};
...
struct amps_trace_context
{
  ...
  amps_trace_buffer                segment_list[AMPS_TRACE_SEGMENT_LIST_MAX_SIZE];
...
/// \param newSize_  the desired number of segments
inline void amps_protocol_output_message_segment_list_reserve(amps_protocol_output_message *pMessage_,size_t newSize_)
{
  amps_message_segment_list_reserve(pMessage_->segment_list,newSize_);
}
...
/// \param length_   the length of the data segment
inline void amps_protocol_output_message_push_back_static_segment(amps_protocol_output_message *pMessage_,const char *pData_,size_t length_)
{
  amps_message_segment_list_push_back_static_segment(pMessage_->segment_list,pData_,length_);
  pMessage_->byte_count += length_;
}
...
inline void amps_protocol_output_message_push_back_segment(amps_protocol_output_message *pMessage_,amps_message_segment *pSegment_)
{
...
/// \param pMessage_ a pointer to the output message
/// \param pSegment_ the segment to push onto the list
/// \param length_   the length of the segment
inline void amps_protocol_output_message_push_back_segment_with_length(amps_protocol_output_message *pMessage_,amps_message_segment *pSegment_,size_t length_)
{
  pSegment_->byte_count = length_;
...
/// \param pSegment_ the segment to push onto the list
/// \param length_   the length of the segment
inline void amps_protocol_output_message_push_back_segment_with_offset(amps_protocol_output_message *pMessage_,amps_message_segment *pSegment_,size_t offset_,size_t length_)
{
  pSegment_->byte_count = length_;
...
/// \param messageSize_ the desired size to hold the serialized sow data
inline void amps_output_message_sow_data_reserve(amps_output_message_sow_data *pSowData_,size_t messageSize_)
{
  if (pSowData_->segment.byte_count + messageSize_ < pSowData_->segment.protocol_buffer.allocated_byte_count) return;

  amps_message_segment *pCurrent = &(pSowData_->segment);
  size_t newSize  = (2 * pCurrent->protocol_buffer.allocated_byte_count) + messageSize_;

  amps_message_buffer buffer;
  amps_message_buffer_allocate(&buffer,newSize);

  if (pCurrent->byte_count) __builtin_memcpy(const_cast<char*>(buffer.data),pCurrent->data,pCurrent->byte_count);
  if (pCurrent->protocol_buffer.deallocate) pCurrent->protocol_buffer.deallocate(pCurrent->protocol_buffer.buffer_handle);
...
///
extern "C"
void amps_set_input_header_field_string(amps_input_message *pMessage_,int fieldId_,const char *pData_,size_t len_);
...
///
extern "C"
void amps_assign_input_header_field_string(amps_input_message *pMessage_,int fieldId_,const char *pData_,size_t len_);
...
///
extern "C"
void amps_set_input_header_field_uint64(amps_input_message *pMessage_,int fieldId_,uint64_t uival_);
...
///
extern "C"
void amps_set_input_header_field_bool(amps_input_message *pMessage_,int fieldId_,int bool_);
...
//  \param len_      the length of the body data
//
extern "C"
void amps_set_input_message_data(amps_input_message *pMessage_,size_t offset_,size_t len_);
...
// framing
typedef int (*amps_protocol_allocate_segment_function)(amps_message_buffer_allocate_function allocate_,amps_message_segment *pSegment_,size_t size_);
typedef int (*amps_protocol_finalize_segment_function)(amps_message_segment *,size_t messageSize_);
...
/// \param size_      the size of the requested segment
/// \returns AMPS_SUCCESS or AMPS_FAILURE
///
int amps_protocol_allocate_segment_function(amps_message_buffer_allocate_function allocate_,amps_message_segment *pSegment_,size_t size_);

///
/// Finalize the framing of a segment to complete a message
///
/// \param pSegment_    a pointer to the segment that will finalized
/// \param messageSize_ the size of the message
/// \returns AMPS_SUCCESS or AMPS_FAILURE
///
int amps_protocol_finalize_segment_function(amps_message_segment *pSegment_,size_t messageSize_);
