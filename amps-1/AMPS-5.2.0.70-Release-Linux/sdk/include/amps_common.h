...
{
  if (pSrc_->protocol_buffer.flags) pSrc_->protocol_buffer.copy(pSrc_->protocol_buffer.buffer_handle);
...
typedef int(*amps_message_type_trace_function)(amps_message_type_context,const struct amps_trace_buffer*,struct amps_trace_output_buffer*);
...
/// AMPS convenience function to ensure that a segment has the desired size
///
/// \param allocator_ the allocator to use if a new segment is required to be allocated
/// \param pSegment_  the segment to reserve
/// \param newSize_   the desired size of the segment
inline void amps_message_segment_expand(amps_message_buffer_allocate_function allocate_,struct amps_message_segment *pSegment_,uint32_t newSize_)
{
  if (newSize_ < pSegment_->protocol_buffer.allocated_byte_count) return;

  struct amps_message_buffer buffer;
  allocate_(&buffer,newSize_);

  if (pSegment_->byte_count) memcpy((char*)(buffer.data),pSegment_->data,pSegment_->byte_count);
  if (pSegment_->protocol_buffer.deallocate) pSegment_->protocol_buffer.deallocate(pSegment_->protocol_buffer.buffer_handle);
  pSegment_->protocol_buffer.buffer_handle = buffer.handle;
  pSegment_->protocol_buffer.deallocate = buffer.deallocate;
  pSegment_->protocol_buffer.copy = amps_message_buffer_copy;
  pSegment_->protocol_buffer.allocated_byte_count = buffer.allocated_byte_count;
  pSegment_->protocol_buffer.flags = 0;
  pSegment_->data = buffer.data;
}

/// AMPS convenience function to ensure that a segment list has at
/// least the specified number of segments
///
/// \param pList_ the segment list to reserve space for
/// \param newSize_ the number of segments to reserve
inline void amps_message_segment_list_reserve(struct amps_message_segment_list *pList_,uint32_t newSize_)
{
  if (newSize_ >= pList_->allocated_segment_count)
   {
     uint32_t i = 0;
     struct amps_message_segment *tmp = pList_->allocate(newSize_);
     for (; i != pList_->segment_count; ++i) tmp[i] = pList_->array[i];
     if (pList_->array != &pList_->static_array[0]) pList_->deallocate(pList_->array);
     pList_->allocated_segment_count = newSize_;
     pList_->array = tmp;
   }
}
...
inline void amps_message_segment_list_push_back_static_segment(struct amps_message_segment_list *pList_,const char *pData_,uint32_t length_)
{
  if (pList_->segment_count < pList_->allocated_segment_count)
   {
     ...
     pSegment->byte_count = length_;
     ++pList_->segment_count;
   }
  else
   {
     amps_message_segment_list_reserve(pList_,pList_->allocated_segment_count << 1);
     amps_message_segment_list_push_back_static_segment(pList_,pData_,length_);
   }
}
...
inline void amps_message_segment_list_push_back_segment(struct amps_message_segment_list *pList_,struct amps_message_segment *pSegment_)
{
  if (pList_->segment_count < pList_->allocated_segment_count)
   {
...
inline void amps_message_segment_list_push_back_segment_with_length(struct amps_message_segment_list *pList_,struct amps_message_segment *pSegment_,uint32_t length_)
{
  if (pList_->segment_count < pList_->allocated_segment_count)
   {
     struct amps_message_segment *pSegment = &pList_->array[pList_->segment_count];
   ...
   {
     amps_message_segment_list_reserve(pList_,pList_->allocated_segment_count << 1);
     amps_message_segment_list_push_back_segment_with_length(pList_,pSegment_,length_);
   }
}
...
/// \param pSegment_ the segment to push onto the list
/// \param length_   the length of the segment
inline void amps_message_segment_list_push_back_segment_with_offset(struct amps_message_segment_list *pList_,struct amps_message_segment *pSegment_,size_t offset_,uint32_t length_)
{
  if (pList_->segment_count < pList_->allocated_segment_count)
   {
     struct amps_message_segment *pSegment = &pList_->array[pList_->segment_count];
   ...
   {
     amps_message_segment_list_reserve(pList_,pList_->allocated_segment_count << 1);
     amps_message_segment_list_push_back_segment_with_offset(pList_,pSegment_,offset_,length_);
   }
}
