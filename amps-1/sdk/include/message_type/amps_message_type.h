...
struct amps_message_value
{
  ...
  /// length of the path
  size_t                   path_length;
  /// a 32 bit hash of the xpath string
  uint32_t                 path_hash;
...
static const uint32_t amps_message_xpath_value_mask_bool   = 0x4;
...
{
  return (pValueSetEntry_ == NULL) || (pValueSetEntry_->hash == 0 && pValueSetEntry_->raw_bytes == 0 && pValueSetEntry_->raw_bytes_length == 0);
}
...
{
  if ( (pValueSetEntry1_->hash == 0 && pValueSetEntry1_->raw_bytes == 0 && pValueSetEntry1_->raw_bytes_length == 0) ||
       (pValueSetEntry2_->hash == 0 && pValueSetEntry2_->raw_bytes == 0 && pValueSetEntry2_->raw_bytes_length == 0) ||
...
typedef uint32_t    amps_message_xpath_hash;
...
struct amps_output_message_data
{
  ...
  /// compute_serialize_size.
  uint32_t                        serialize_size;        // set by message_type in compute_serialize_size or if segment buffer is too small
...
inline void amps_delta_message_data_reserve(amps_delta_message_data *pMessage_,size_t len_)
{
  ...
  if (pSegment->byte_count + len_ < pSegment->protocol_buffer.allocated_byte_count) return;

  uint32_t newSize = 2 * pSegment->protocol_buffer.allocated_byte_count;
  if (pSegment->byte_count + len_ >= newSize) newSize += len_;
...
///
/// Invoked by AMPS when the xpath_value_set where this user data is stored is being cleared.
/// \param userData user data set by user on a value set
///
void amps_message_type_free_user_data_function(void *userData_);
...
///
amps_message_xpath_hash amps_message_xpath_hash_string(const char *pString_ ,size_t len_, amps_message_xpath_hash baseHash_);
...
///
/// Sets the user data pointer on this xpath_value_set. The user data
/// pointer is an arbitrary value that can be used to save state
/// between header and body parses of a message. For example, if your
/// parser creates an in-memory tree of the entire message content
/// when performing a partial parse, you can save a pointer to it in
/// user data and then retrieve it once again if and when full parse
/// is called; likewise after a full parse, you may be able to reuse
/// parsing artifacts to speed up a delta or merge operation.
///
/// \param valueSet_ the amps_message_xpath_value_set handle to store the user data in.
/// \param userData_ the user data to store on this
///                   amps_message_xpath_value_set While this is a
///                   void*, you are welcome to store any 64-bit value
///                   here.
///
void amps_message_xpath_set_user_data(amps_message_xpath_value_set valueSet_,void* userData_);

///
/// Returns the user data set on this
/// amps_message_xpath_value_set. See documentation for
/// amps_message_xpath_set_user_data for more information.
///
/// \param valueSet_ the amps_message_xpath_value_set handle to retrieve the user data from.
/// \returns the user data pointer (or other 64-bit value), or 0 if no
///          user data has been set on valueSet_.
///
...
///
uint32_t amps_message_xpath_find_by_hash(amps_message_xpath_value_set valueSet_, amps_message_xpath_hash hash_);
...
///
uint32_t amps_message_xpath_update(amps_message_xpath_value_set valueSet_,amps_message_xpath_find_set findSet_,amps_message_value_set_entry* pEntry_);
...
///
int amps_message_type_supports_all_options(
    amps_message_type_context, const char**, size_t*);
///
/// Convienience implementation of get_supported options that returns
///  "delta,view" as your message type's supported options.
///
int amps_message_type_supports_delta_and_view(
    amps_message_type_context, const char**, size_t*);

int amps_message_type_default_trace_message(
    amps_message_type_context, const amps_trace_buffer*,
