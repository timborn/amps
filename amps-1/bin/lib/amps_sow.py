...
class Reader20:
   ...
   def __init__(self, path, file_obj, header):
      ...
      self.version = self.version.strip(chr(0))  # filter out the null bytes
...
class Reader30:
   ...
   def __init__(self, path, file_obj, header):
      ...
      self.crc = int(crc)
      self.raw_version = self.version.strip(chr(0))
      self.version = 3.0
...
class Reader40:
   ...
   def records(self):
      ...
      while self.file_obj:
         magic = '00000000'
         slab_size = long(0xffffffffffffffff)
         file_offset = long(0xffffffffffffffff)
         crc = int(0xffffffff)
...
class Reader60(Reader50):
   ...
   def __init__(self, path, file_obj, header, validate=True):
      ...
      self.raw_version = self.version.strip(chr(0))
      self.raw_compression_type = self.compression_type.strip(chr(0))
      self.version = 6.0
...
def version(sow_file):
   ...
   if sow_meta_data.find('amps::amps_pstore/1.0') == 0:
      return 1
   elif sow_meta_data.find('persist::gpstore/1.0') == 0:
      return 2
   elif sow_meta_data.find('persist::gpstore/1.1') == 0:
      return 2.1
   elif sow_meta_data.find('persist::gpstore/3.0') == 0:
      return 3.0
   elif sow_meta_data.find('amps-sow-v1.0') == 0:
      return 4.0
   elif sow_meta_data.find('amps-sow-v2.0') == 0:
      return 5.0
   elif sow_meta_data.find('amps-sow-v3.0') == 0:
      return 6.0
   else:
      return -1
