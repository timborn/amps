...
#
def dump(file, start, stop, is_localtime, is_extents, omit_data=False):
   ...
   for jnl_path in Fileset(file).files():
      ...
      for record in jnl_file.records():
         if not record["typeStr"] == 'noop':
             if total_entry_count >= start and total_entry_count <= stop:
             ...
             if len(record['messageTypeName']) > 10:
                 total_unknown_command_count += 1
         jnl_file_byte_count += record['size']
      total_byte_count += (jnl_file_byte_count + AMPS_TX_OFFSET)
      ...
      if source_extents_count:
         ...
         for k,v in source_extents.iteritems():
...
Total Bytes      : {1}
Remaining Bytes  : {2}
   """.format(total_entry_count, total_byte_count, total_remaining_count)
...
iso8601 timestamp   : {iso8601Timestamp:>30}TZSUFFIX
...
rep path            : [{replicationPath}] """
      ...
      formatstr = formatstr.replace("TZSUFFIX", tz_suffix)
...
class File:
   ...
   def __init__(self, filename):
      ...
      if(self.version >= 7):
         ...
         self.compressionType = self.compressionType.strip(chr(0)).strip()

      if(not self.magic == "AMPS"):
      ...
      self.versionString = self.versionString.strip(chr(0)).strip()
      if(self.version == 1):
   ...
   def records(self):
      ...
      while(len(self.record) == self.min_tx_size):
         ...
         if(self._crc == 0 or self._size == 0):
            break
         # Read remaining
         self.remainingBlocks = (self._size/self.min_tx_size) - 1;
         ...
         self.flagsStr = {0: 'none',
                          1: 'compressed',
                          2: 'message queue',
                          3: 'compressed, message queue',
                          4: 'string key',
                          5: 'compressed, string key',
                          6: 'message queue, string key',
                          7: 'compressed, message queue, string key'}.get(self._flags, 'none')
         ...
         yield Record({ 'crc': self._crc, 'size': self._size, 'type': self._type, 'messageLen': self._messageLen,
               ...
               'topic': self.topic, 'data': self.data, 'authId': self._authId, 'replicationPath': self._replicationPath, 'typeStr':self.type, 'version':self.version,
               'entryCount': self.entryCount, 'iso8601Timestamp': File.iso8601_time(self._timestamp), 'sowExpiration':self._sowExpiration,
               'previousSize':self._previousSize, 'correlationId': self._correlationId, 'correlationIdLen':self._correlationIdLen,
               'key': self._key, 'keyLen':self._keyLen, 'offset':self.offset })
...
Version         : {2:20}
Extents         : {4:20}
First Timestamp : {5:20}TZSUFFIX
Last Timestamp  : {6:20}TZSUFFIX
_____________________________________________________"""
      ...
      if (self.version < 7):
         ...
         first =  File.iso8601_time(self.first_timestamp)
         last = File.iso8601_time(self.last_timestamp)
...
class Writer:
   ...
   def pack(self, struct, crc, record):
       return struct.pack(crc,                          # 00
                         ...
                         record['authId'],              # 22
                         record['key'],                 # 23
                         record['correlationId'],       # 24
                         record['replicationPath'],     # 25
                         record['data'])                # 26
   ...
   def write(self, record):
      ...
      if len(recordBytes) != record['size']:
          bytesToWrite = AMPS_MIN_TX_SIZE - (len(recordBytes) % AMPS_MIN_TX_SIZE)
          record['size'] = len(recordBytes) + bytesToWrite
          write_record_struct = struct.Struct(format_str)
          recordBytes = self.pack(write_record_struct, write_crc, record)
      ...
      self.write_file.write(recordBytes)
      bytesToWrite = AMPS_MIN_TX_SIZE - (len(recordBytes) % AMPS_MIN_TX_SIZE)
      if bytesToWrite != 0 and bytesToWrite != AMPS_MIN_TX_SIZE:
